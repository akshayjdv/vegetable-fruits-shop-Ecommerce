...
const UpdateProduct = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (productname && category && image && price && description) {
      ...
      if (!result?.success) {
        setData("");
        toast("error while updating new product " + result?.message);
      }
  ...
  return (
    <div>
      <div className="p-4">
        <form
          className="m-auto w-full max-w-md drop-shadow shadow flex flex-col p-3 bg-white"
        ...
        >
          ...
          <label htmlFor="image">
            ...
            <div className="h-40 w-full bg-slate-300 my-1 rounded flex items-center justify-center cursor-pointer">
              {data.image ? (
                <img src={data.image} alt="product-photu" className="h-full" />
              ) : (
